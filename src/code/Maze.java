 ...
 *
 * ASSIGNMENT 2-Part2
 * STUDENT AUTHOR: Muhammed Burak Altun
 *
 * MODIFY
 *
 * */
    ...
    public boolean solveMaze(iSimpleContainer<Coordinate> sc, iDeque<Coordinate> visitedNodes, String mazeName)
        ...
        while(!sc.isEmpty()) {
            ...
            if(isExit(currentState.x,currentState.y)) {
                ...
                Coordinate upperNeighbor = new Coordinate(currentState.x, currentState.y - 1);
                Coordinate rightNeighbor = new Coordinate(currentState.x + 1, currentState.y);
                Coordinate lowerNeighbor = new Coordinate(currentState.x, currentState.y + 1);
                Coordinate leftNeighbor = new Coordinate(currentState.x - 1, currentState.y);

                if(isInMaze(upperNeighbor) && currentMaze[upperNeighbor.x][upperNeighbor.y] != I && !isVisited(upperNeighbor)) {
                    sc.push(upperNeighbor);
                }
                if(isInMaze(rightNeighbor) && currentMaze[rightNeighbor.x][rightNeighbor.y] != I && !isVisited(rightNeighbor)) {
                    sc.push(rightNeighbor);
                }
                if(isInMaze(lowerNeighbor) && currentMaze[lowerNeighbor.x][lowerNeighbor.y] != I && !isVisited(lowerNeighbor)) {
                    sc.push(lowerNeighbor);
                }
                if(isInMaze(leftNeighbor) && currentMaze[leftNeighbor.x][leftNeighbor.y] != I && !isVisited(leftNeighbor)) {
                    sc.push(leftNeighbor);
                }
            }
        }
        return false;
